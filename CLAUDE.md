...
- **data/flavorDescriptions.ts**: フレーバー詳細ページ「Tasting note」用の日本語説明 (client-safe)。キーは `flavorDescriptionKey(manufacturer, productName)` (`brandSlug` + `normalizeFlavorName` 複合) で、同一フレーバーのサイズ違い id をまとめてカバー。curatedPicks の `note` へ id フォールバックあり。`getFlavorDescription()` が `null` なら UI はセクションごと非表示 — 公知のテイスト情報のみ記載し、捏造しないこと。
...
- **data/flavorTagTaxonomy.ts**: フレーバータグの固定タクソノミー (18種、client-safe)。自由記述タグは追加しないこと。
- **lib/tags/flavorTagger.ts**: 商品名 → タグのルールエンジン。キーワード辞書 (単語境界付き英語 + 部分一致カタカナ、長いパターン優先で消費) + `data/flavorTagOverrides.ts` (辞書で判定できない固有名のAI分類結果、`normalizeFlavorName` キー)。**オーバーライドは根拠のあるものだけ — 不明な名前は未タグのまま**。実行はビルド時のみで、`scripts/build-data.ts` が `data/generated/flavorTags.json` (id → タグ配列) を生成する。
- **data/flavorTags.ts**: `flavorTags.json` のランタイムアクセサ。`getFlavorTags(id)` / `attachFlavorTags(flavor)` (APIレスポンス整形用)。タグは `/api/search` の `tags` パラメータ (カンマ区切り、AND) と searchIndex の全文テキスト (和名/英名ラベル) の両方に効く。
